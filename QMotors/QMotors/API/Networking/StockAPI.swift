        ...
        BaseAPI.authorizedGetRequest(reqMethod: .stockList, parameters: params, success: { data in
            ...
            if errors.type == .null {
                ...
                for stock in jsonData["result"].arrayValue {
                    stocks.append(Stock(id: stock["id"].intValue, title: stock["title"].stringValue, subtitle: stock["subtitle"].stringValue, location: stock["location"].string, description: stock["description"].stringValue, created_at: stock["created_at"].stringValue, updated_at: stock["updated_at"].stringValue))
                }
                success(stocks)
            } else {
