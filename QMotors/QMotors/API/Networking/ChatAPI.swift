...
final class ChatAPI {
    ...
    static func sendPhoto(data: Data, success: @escaping (JSON) -> Void, failure: @escaping escapeNetworkError) {
        BaseAPI.authorizedMultipartPostChatPhoto(data: data) { data in
            guard let data = data else { return }
